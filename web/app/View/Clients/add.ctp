		...
		<fieldset ng-init="urlTown='<?php echo $this->Html->url(array('controller' => 'towns', 'action' => 'getTown', 'admin' => false)) ?>'">
			...
			<div class="form-group" ng-show="existe">
				<div class="classForm corpsForm">
					...
					<select class="form-control input-sm" style="width:164px; margin-left:100px;" ng-model="Clients.Client.town_id" ng-init="initTown2()" name="town_id" ng-options="value.Town.name for value in villes track by value.Town.id" ng-change="updateZipCode2()" required></select>
				</div>
			</div>
			...
			<br>
			<div class="form-group classForm corpsForm">
				<label for="association_id" class="col-sm-4 control-label" style="width : 150px; margin-left : -45px;">Association : </label>
				<?php echo $this->Form->select('association_id', $listAssoc, array('class' => 'form-control input-sm', 'style' => 'width: 200px;', 'name' => 'association_id')); ?>
			</div>
